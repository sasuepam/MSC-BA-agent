...
1. Ask the user for the **requirement ID** if not already provided (format: `NEW-XXXX`, e.g. `NEW-0042`). Do not proceed without it.
2. Read all provided input materials (files, pasted text, URLs via WebFetch, Confluence pages via MCP tool if available). **Do NOT read files from `output/specs/`, `output/stories/`, or `output/validation/` unless the caller has explicitly named one of those files as an input. Those directories contain previous outputs for other features and must not be used to infer format or content.**
3. Identify: the overall solution being delivered, and the specific API(s) the MuleSoft team must build or change to support it.
4. Extract: feature name, actors, business rules, solution use cases, API names/operations, and any known gaps.
5. Derive a `feature_name` slug (lowercase, underscores, no spaces) from the feature title.
6. Run `mkdir -p "output/specs"` to ensure the output directory exists.
7. Write the completed spec to `output/specs/functional_spec_<req-id>_[feature_name].html` (e.g. `functional_spec_NEW-0042_timezone_management.html`).
8. Report the saved file path and a brief list of any fields marked TO BE CONFIRMED.
