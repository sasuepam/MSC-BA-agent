...
Call `mcp__msc-ba__confluence_get_page` with the page ID.
...
Call `mcp__msc-ba__confluence_get_author_info` before building the row. This returns:
...
- **AUTHOR(S):** use `author_cell_html` from `mcp__msc-ba__confluence_get_author_info` verbatim. This will render as:
...
- **STATUS:** use `status_cell_html` from `mcp__msc-ba__confluence_get_author_info` verbatim. This renders as a blue **Draft** status label macro — do not use plain text.
...
Before calling `mcp__msc-ba__confluence_update_page`, show the user:
