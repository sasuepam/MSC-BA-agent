...
- Spec files: `output/specs/*.html`
- Story files: `output/stories/*.html` and `output/stories/*.md`

If the user names specific files, validate only those. Otherwise validate all files found in both directories.

**Spec-to-story correlation rule:**
For each spec file, extract the requirement ID by taking the segment of the filename between `functional_spec_` and the next `_` (e.g. `functional_spec_NEW-0042_timezone_management.html` → `NEW-0042`). The corresponding story files for that spec are all files in `output/stories/` whose filename begins with that requirement ID (e.g. `NEW-0042-timezone-cr-001.md`, `NEW-0042-timezone-us-001.md`). Use this mapping for Rules 5, 7, and 8 that require cross-referencing spec content against generated stories.
...
Ensure the output directory exists: `output/validation/`

Save the report to:
`output/validation/validation-report.md`
