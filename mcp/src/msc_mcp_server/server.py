...
async def app_lifespan(server: FastMCP) -> AsyncIterator[AppContext]:
    ...
    logger.info("MSC BA MCP Server started — transport: %s", settings.transport)
    try:
        yield ctx
    finally:
        for client in ctx.http_clients.values():
            await client.aclose()
        logger.info("MSC BA MCP Server stopped")
...
def main():
    """CLI entry point."""
    parser = argparse.ArgumentParser(description="MSC BA MCP Server")
    parser.add_argument(
        "--transport",
    ...
    logger.info("Starting MSC BA MCP Server on %s:%s (transport=%s)", args.host, args.port, args.transport)
