...
Present this sub-menu and wait for their choice:

---

**Stories only — how would you like to provide input?**

a. **From spec** — generate stories from an existing functional spec file
b. **Direct input** — provide interface names and requirements directly (no spec file needed)

---

#### Choice 2a — From spec
List the available files in `output/specs/` so the user can choose, then ask:
"Which spec file should I use? (leave blank to use the most recent)"
Set `INPUT_SOURCE="spec"`.
Wait for their response, then proceed to [PHASE: STORIES].

#### Choice 2b — Direct input
Ask: "Please provide the interface names and requirements you want stories generated for. Include: interface name(s), whether each is new or an existing change, and any relevant acceptance criteria or business rules."
Set `INPUT_SOURCE="direct"`.
Wait for their response, then proceed to [PHASE: STORIES], passing the user's text directly to the ba-story-generator agent instead of a spec file.
...
m = {
  ...
  'phases': {
    'spec': {
      ...
      'iterations': 0,
      'output_file': None,
      'template_auto_fixes': 0,
      'template_manual_fixes': 0
    },
    'stories': {
      'started_at': None,
      'completed_at': None,
      ...
      'us_count': None,
      'input_source': None,
      'stories_auto_fixed': 0,
      'stories_manual_fixed': 0
    },
    'validation': {'runs': []},
    'amend': {'runs': []},
...
SPEC_FILE="[path to saved spec file, e.g. output/specs/functional_spec_NEW-0042_timezone_management.html]"
python3 -c "
import json, re
...
# Extract requirement ID from spec filename (segment between functional_spec_ and next _)
match = re.search(r'functional_spec_([^_]+)_', '${SPEC_FILE}')
if match:
    m['feature_requirement_id'] = match.group(1)
...
INPUT_SOURCE="spec"  # set to "direct" if invoked via Choice 2b (direct input); "spec" for all other paths
python3 -c "
import json
with open('${METRICS_FILE}') as f: m = json.load(f)
...
**If INPUT_SOURCE is "spec":** Invoke the `ba-story-generator` agent with the spec file from `output/specs/`.
**If INPUT_SOURCE is "direct":** Invoke the `ba-story-generator` agent with the user's raw interface names and requirements text.

Wait for it to complete and confirm all story file paths saved in `output/stories/`.

### METRICS: Record stories complete
After the agent reports the CR, US counts, and file paths, run:
```bash
STORIES_END=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
STORIES_FILES='["path1.md", "path2.md"]'  # JSON array of all story file paths reported by the generator
...
m['phases']['stories']['output_files'] = ${STORIES_FILES}
...
**If invoked from Choice 2 (Stories only):**

Do a lightweight template compliance check on each story file just saved: read the file and check whether any required template fields are blank or still contain `[TO BE CONFIRMED]`. Count issues per file.

Print the following summary and **stop** — do not prompt for validation or publishing:
...
| ADF interfaces excluded | [n] ([list names, or "None"]) |
| Story files saved | [list paths] |

Template compliance per story:
- [filename]: [Pass / n issue(s) found — resolve with /ba-amend after running ba-validator]

Stories are saved in output/stories/. Run /ba-workflow option 4 when you are ready to validate and publish.
```

**If running as part of a full end-to-end (Choice 3):** automatically continue to [PHASE: VALIDATE].

**If invoked from Choice 1 (Spec only, continuing to stories after confirmation):** ask: "Continue to validation? (yes / no)"
...
Read the counts from the validator's summary, split by structural vs content rule classification:

- **Structural rules** (wrong construction of output): Rules 3, 4, 5, 8
- **Content rules** (missing or incomplete content): Rules 1, 2, 6, 7

Parse the saved validation report to count flags per rule number, then run:

```bash
VAL_END=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
STRUCT_BLOCKERS=[count of BLOCKER flags from rules 3,4,5,8]
STRUCT_WARNINGS=[count of WARNING flags from rules 3,4,5,8]
CONTENT_BLOCKERS=[count of BLOCKER flags from rules 1,2,6,7]
CONTENT_WARNINGS=[count of WARNING flags from rules 1,2,6,7]
CONTENT_INFOS=[count of INFO flags from rules 1,2,6,7]
python3 -c "
import json
with open('${METRICS_FILE}') as f: m = json.load(f)
...
Once all flags have been handled, the amend skill will print its summary table including Structural fixes and Content fixes counts.

### METRICS: Record amend complete and increment feedback loop
Read the counts from the amendment summary table, then run:
```bash
AMEND_END=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
APPLIED=[number applied]
EDITED=[number edited]
SKIPPED=[number skipped]
STRUCTURAL_FIXES=[number of structural fixes from summary]
CONTENT_FIXES=[number of content fixes from summary]
python3 -c "
import json
with open('${METRICS_FILE}') as f: m = json.load(f)
